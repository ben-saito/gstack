...
実際のChromiumブラウザからログイン済みセッションをヘッドレスbrowseセッションにインポートする。

## 仕組み

1. browseバイナリを見つける
2. `cookie-import-browser`を実行してインストール済みブラウザを検出し、ピッカーUIを開く
3. ユーザーがブラウザでインポートするCookieドメインを選択する
4. Cookieが復号化されPlaywrightセッションに読み込まれる
...
インストール済みのChromiumブラウザを自動検出し、デフォルトブラウザでインタラクティブなピッカーUIを開く。以下の操作が可能：
- インストール済みブラウザの切り替え
- ドメインの検索
- 「+」をクリックしてドメインのCookieをインポート
- ゴミ箱をクリックしてインポート済みCookieを削除

ユーザーに伝える：**「Cookieピッカーが開きました — ブラウザでインポートしたいドメインを選択して、完了したら教えてください。」**

### 3. 直接インポート（代替方法）

ユーザーがドメインを直接指定した場合（例：`/setup-browser-cookies github.com`）、UIをスキップする：

```bash
$B cookie-import-browser comet --domain github.com
```

指定された場合、`comet`を適切なブラウザに置き換える。
...
インポートされたCookieのサマリー（ドメイン別のカウント）をユーザーに表示する。

## 注意事項

- macOSでは、ブラウザごとの初回インポート時にキーチェーンダイアログが表示される場合がある — 「許可」/「常に許可」をクリックする
- Linuxでは、`v11`のCookieに`secret-tool`/libsecretアクセスが必要な場合がある。`v10`のCookieはChromiumの標準フォールバックキーを使用する
- Cookieピッカーはbrowseサーバーと同じポートで提供される（追加プロセスは不要）
- UIにはドメイン名とCookieの数のみが表示される — Cookie値は公開されない
- browseセッションはコマンド間でCookieを保持するため、インポートしたCookieはすぐに機能する
